...
function! s:start_spinner(buf, message) abort
  let g:tmc_spinner_buf = a:buf
  let g:tmc_spinner_message = a:message
  call setbufline(a:buf, 1, s:spinner_frames[0] . ' ' . a:message)
  let g:tmc_spinner_timer = timer_start(100, 's:spinner_tick', {'repeat': -1})
endfunction

function! s:stop_spinner() abort
  if exists('g:tmc_spinner_timer')
    call timer_stop(g:tmc_spinner_timer)
    unlet g:tmc_spinner_timer
  endif
  if exists('g:tmc_spinner_buf') && bufloaded(g:tmc_spinner_buf)
    call deletebufline(g:tmc_spinner_buf, 1)
  endif
endfunction

function! s:spinner_tick(timer) abort
  if !exists('g:tmc_spinner_buf') || !bufloaded(g:tmc_spinner_buf)
    call timer_stop(a:timer)
    return
  endif
  let frame = s:spinner_frames[s:spinner_index]
  let s:spinner_index = (s:spinner_index + 1) % len(s:spinner_frames)
  call setbufline(g:tmc_spinner_buf, 1, frame . ' ' . g:tmc_spinner_message)
endfunction
        ...
        attach_mappings = function(_, map)
          actions.select_default:replace(function(bufnr)
            ...
            vim.schedule(function()
              vim.fn.call('tmc#ui_call_callback', {vim.g.TMC_UI_CB_ID, entry and entry[1] or ''})
            end)
            actions.close(bufnr)
          end)
          return true
...
lua << EOF
      if vim.ui and type(vim.ui.select) == 'function' then
        vim.ui.select(vim.g.tmc_ui_select_items, { prompt = vim.g.tmc_ui_select_prompt }, function(choice)
          vim.schedule(function()
            vim.fn.call('tmc#ui_call_callback', {vim.g.TMC_UI_CB_ID, choice or ''})
          end)
        end)
      end
EOF
  ...
  " fzf.vim (works in both Vim and Neovim)
  ...
  " Vim popup_menu()/inputlist() fallback
    ...
    call call('tmc#ui_call_callback', [l:cb_id, l:choice])
    return v:null
  endif
endfunction
...
function! tmc#run_tests_current() abort
  ...
  call s:start_spinner(g:tmc_test_buf, 'Running tests...')
  ...
  elseif exists('*job_start')
    call job_start(l:cmd, {
          \ 'out_cb': function('s:RunTests_on_output_vim'),
          \ 'err_cb': function('s:RunTests_on_output_vim'),
          \ 'exit_cb': function('s:RunTests_finalize_vim'),
          \ })
  else
    call s:run_tests_fallback(l:cmd)
  endif
endfunction
...
function! s:RunTests_on_output(job_id, data) abort
  for line in a:data
    ...
    if line =~ '{' && line =~ 'test-result'
      let g:tmc_test_json = line
    else
      call add(g:tmc_test_logs, '⏳ ' . line)
    endif
  endfor
endfunction

function! s:RunTests_on_output_vim(channel, msg) abort
  call s:RunTests_on_output(0, split(a:msg, "\n"))
endfunction

function! s:RunTests_finalize(job_id, code, event) abort
  call s:stop_spinner()
  call s:print_test_results()
endfunction

function! s:RunTests_finalize_vim(channel, code) abort
  call s:stop_spinner()
  call s:print_test_results()
endfunction

function! s:run_tests_fallback(cmd) abort
  let l:objs = tmc#run_cli_streaming(a:cmd)
  for obj in l:objs
    if get(obj, 'output-kind', '') ==# 'output-data'
      let g:tmc_test_json = json_encode(obj)
    endif
  endfor
  call s:print_test_results()
endfunction

function! s:print_test_results() abort
  ...
  let results = get(obj['data']['output-data'], 'testResults', [])
  let failed = 0
  ...
  for tc in results
    if get(tc, 'successful', v:false)
      call appendbufline(g:tmc_test_buf, '$', '✅ ' . tc['name'])
    else
      let failed += 1
...
" Submit current exercise with async support for both Neovim and Vim8
function! tmc#submit_current() abort
  ...
  let g:tmc_submit_logs = []

  " Start spinner
  call s:start_spinner(g:tmc_submit_buf, 'Submitting exercise...')
  ...
  if exists('*jobstart')  " Neovim
    call jobstart(l:cmd, {
          ...
          \ 'on_exit':   function('s:Submit_on_exit_shared'),
          \ 'stderr':    'ignore',
          \ })
  elseif exists('*job_start')  " Vim8
    call job_start(l:cmd, {
          \ 'out_cb': function('s:Submit_on_stdout_vim'),
          \ 'exit_cb': function('s:Submit_on_exit_vim'),
          \ })
  endif
endfunction
...
function! s:Submit_on_stdout_pretty(job_id, data, event) abort
  for line in a:data
    ...
    try
      ...
      call add(g:tmc_submit_logs, line)
      continue
    endtry

    if get(obj, 'output-kind', '') ==# 'status-update'
      call add(g:tmc_submit_logs, printf('⏳ %3.0f%% %s', obj['percent-done'] * 100, obj['message']))
...
" Handle stdout (Vim8)
function! s:Submit_on_stdout_vim(channel, msg) abort
  if empty(a:msg) | return | endif
  try
    let obj = json_decode(a:msg)
  catch
    call add(g:tmc_submit_logs, a:msg)
    return
  endtry
...
" Shared exit handler (called by both Neovim and Vim8)
function! s:Submit_on_exit_shared(job_id, code, event) abort
  call s:stop_spinner()
  call s:render_submit_results()
endfunction

function! s:Submit_on_exit_vim(channel, code) abort
  call s:stop_spinner()
  call s:render_submit_results()
endfunction

" Render results for both Vim/Neovim
function! s:Render_submit_results() abort
  if !exists('g:tmc_submit_buf') || !bufloaded(g:tmc_submit_buf)
    return
  endif

  " Print logs
  if !empty(g:tmc_submit_logs)
    call appendbufline(g:tmc_submit_buf, '$', '--- Progress ---')
    call appendbufline(g:tmc_submit_buf, '$', g:tmc_submit_logs)
    let g:tmc_submit_logs = []
  endif
...
function! s:Submit_on_exit_vim(channel, code) abort
  call s:Submit_on_exit_pretty(0, [], 'exit')
endfunction
...
function! tmc#download_course_exercises(course_id, org, cb) abort
  ...
  " Start spinner
  call s:start_spinner(g:tmc_download_buf, 'Downloading exercises...')
  ...
  elseif exists('*job_start')
    call job_start(l:args, {
          ...
          \ 'exit_cb': {ch, code -> s:Download_on_exit_vim(ch, code, a:course_id, a:cb)},
          \ })
  else
    call s:download_course_fallback(l:args, a:course_id, a:cb)
  endif
endfunction
...
function! s:Download_on_stdout_vim(channel, msg) abort
  call s:Download_on_stdout_pretty(0, split(a:msg, "\n"), '')
endfunction

" Async exit handler (Neovim)
function! s:Download_on_exit_pretty(job_id, code, event, course_id, cb) abort
  call s:stop_spinner()
  call s:print_download_summary(a:course_id)
  call a:cb(a:course_id)
endfunction

" Vim8 exit handler
function! s:Download_on_exit_vim(channel, code, course_id, cb) abort
  call s:stop_spinner()
  call s:print_download_summary(a:course_id)
  call a:cb(a:course_id)
endfunction
...
" Sync fallback
function! s:download_course_fallback(args, course_id, cb) abort
  let l:objs = tmc#run_cli_streaming(a:args)
  for obj in l:objs
    if get(obj, 'output-kind', '') ==# 'output-data'
      let g:tmc_last_download_result = obj
    endif
...
" Summary printing
function! s:print_download_summary(course_id) abort
  if !exists('g:tmc_download_buf') || !bufloaded(g:tmc_download_buf)
    return
  endif
